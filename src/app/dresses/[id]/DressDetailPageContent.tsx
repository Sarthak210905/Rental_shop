...
import { Dress, Product } from "@/lib/mock-data";
...
import { getDressById } from "@/lib/services/productService";
...
interface DressDetailPageContentProps {
  dressId: string;
  initialDress: Dress | null;
  relatedProducts: Product[];
}

export default function DressDetailPageContent({
  dressId,
  initialDress,
  relatedProducts,
}: DressDetailPageContentProps) {
  const { addToCart } = useCart();
  const { toast } = useToast();

  const [dress, setDress] = useState<Dress | null>(initialDress);
  const [loading, setLoading] = useState(!initialDress);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(dressRef, (docSnap) => {
      if (docSnap.exists()) {
        setDress({ id: docSnap.id, ...docSnap.data() } as Dress);
      } else {
        console.error("Dress document not found in real-time listener.");
        setDress(null);
      }
      setLoading(false);
    }, (error) => {
      console.error("Error listening to dress document:", error);
      setLoading(false);
    });
  ...
  const isDateBlocked = (day: Date) => {
    ...
    return (dress?.unavailableDates || []).includes(dayString);
  };
  ...
  const isAddToCartDisabled = () => {
    if (!dress || !dress.availability) return true;
    if (bookingType === 'range') {
    ...
    <div className="container mx-auto px-4 py-8 md:py-16">
      <div className="grid md:grid-cols-2 gap-8 lg:gap-16">
        ...
        <div>
          ...
          >
            {dress.availability ? "Add to Cart" : "Currently Unavailable"}
          </Button>
